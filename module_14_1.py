...
cursor.execute('''CREATE TABLE IF NOT EXISTS Users(id INTEGER PRIMARY KEY, username TEXT NOT
NULL, email TEXT NOT NULL, age INTEGER, balance TEXT NOT NULL)''')

for i in range(1, 11):
    cursor.execute('INSERT INTO Users (username, email, age, balance) VALUES(?, ?, ?, ?)', (f'User{i}', f'example{i}@gmail.com', str(i * 10), '1000'))

for i in [j for j in range(1, 11, 2)]:
    cursor.execute('UPDATE Users SET balance = ? WHERE id = ?', ('500', str(i)))

for i in [j for j in range(1, 11, 3)]:
